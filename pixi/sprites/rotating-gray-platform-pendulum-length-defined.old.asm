...
!Chain_Prop = $32
!Platform_Prop = $32 ; also affects the ball behind the platform
!Chain_Tile = $8E
;!Num_Balls = 4 ; >5 may have disappearing chain at left edge of screen
;!Radius #= !Num_Balls<<4
!balls_max = $05
;assert !Radius < $80

warnings enable W1013 ; Implicitly sized immediate
...
rgpp_init:
			print "INIT ",pc
;			LDA.b #!Radius			;\ set platform radius
			LDA !extra_byte_1,x
			CMP.b #(!balls_max+$01)
			BCC .small_enuff                ; \ enforce old assertion
			LDA.b #!balls_max               ; | is still true
			STA !extra_byte_1,x             ; /
.small_enuff
			ASL #4
			STA !187B,x			;/ radius
...
;			LDA #$00
;else
			LDA.b #$03        ; probably fine for plats anyway
;endif
            %SubOffScreen()
...
CODE_02D74B:
			...
			BEQ .cont		;/ return
			JMP Return02D806       ; branch out of bounds
.cont:

			; $0C-$0D: x increment from platform to center of rotation
			LDA !RAM_SpriteXLo,x
			SEC
			SBC 4,s
			JSR CODE_02D870
			...
			; $02 = onscreen y pos << 2
			; we use 14.2 fixed point to prevent inter-frame jitter
			LDA $01
			ASL
			STZ $03
			BCC +
...
;			LDX.b #!Num_Balls-1-1 ; loop index; one ball is drawn already
			LDA !extra_byte_1,x
			DEC : DEC
			TAX
			REP #$20
CODE_02D7AF:
			...
			;LDA.b #!Num_Balls+3-1			; num tiles were drawn
			LDA !extra_byte_1,x
			CLC : ADC #$02
...
Tilemap:		db !Chain_Tile,$8A,$8B,$8C
...
CODE_02D84E:		LDA $00				;\ set tile x position
			...
			STA !OAM_Tile,y			;/
			LDA.b #!Platform_Prop			;\ set tile properties
			STA !OAM_Prop,y			;/
...
;			LDA.b #!Num_Balls
			...
			LDA $4214			; low byte of result
            endif

			PLP				; retrieve processor flags
			BPL +		;\ if original value was negative,
