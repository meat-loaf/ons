...
incsrc "../../asm/headers/ram.asm"
incsrc "pointer_caller.asm"
incsrc "spriteset/spritesets.asm"
; ---------------------------------------------------
...
org $02FFE2
	db "STSD"						;header!
	incbin "/tmp/_versionflag.bin"	;byte 1 is version number 1.xx
										;byte 2 are flags ---- ---l
...
if !SA1 == 0
	...
	TYA
	CLC : ADC.l .byte_tables_2-4,x
	TAY
	LDA.l .byte_tables-4,x
	STA.b $8A
	LDA.l .byte_tables-4+1,x
	STA.b $8B
	LDA.b #.byte_tables>>16
	STA.b $8C
	PLX
	JMP.w [$008A|!dp]
.twelve
	LDA [$CE],y
	STA !spr_extra_byte_12,x
	DEY
.eleven
	LDA [$CE],y
	STA !spr_extra_byte_11,x
	DEY
.ten
	LDA [$CE],y
	STA !spr_extra_byte_10,x
	DEY
.nine
	LDA [$CE],y
	STA !spr_extra_byte_9,x
	DEY
.eight
	LDA [$CE],y
	STA !spr_extra_byte_8,x
	DEY
.seven
	LDA [$CE],y
	STA !spr_extra_byte_7,x
	DEY
.six
	LDA [$CE],y
	STA !spr_extra_byte_6,x
	DEY
.five
	LDA [$CE],y
	STA !spr_extra_byte_5,x
	DEY
.four
	LDA [$CE],y
	STA !spr_extra_byte_4,x
...
.one
	LDA [$CE],y
	STA !spr_extra_byte_1,x
	DEY #3               ; restores it to its original value
	PLA
	JML $02A950|!BankB
.skipExtraByte
	PLX
...
TableStart:
	...
	incbin "/tmp/_DefaultTables.bin"
CustomStatusPtr:
	print "Custom status pointers at", pc
	incbin "/tmp/_CustomStatusPtr.bin"
...
if !PerLevel = 1
	...
	PerLevelLvlPtrs:
		...
		incbin "/tmp/_PerLevelLvlPtrs.bin"
	freedata
	...
	PerLevelCustomPtrTable:
		...
		incbin "/tmp/_PerLevelCustomPtrTable.bin"
	freedata
        skip -1
	PerLevelSprPtrs:
        skip 1
		...
		incbin "/tmp/_PerLevelSprPtrs.bin"
endif
