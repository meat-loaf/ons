...
incsrc "/tmp/__temp_settings.asm"	; settings set by GPS
...
if !__insert_pages_40_plus == 0		; Yeah, too lazy to think of a better method so just throw it in here if it's not 32KB
	block_bank_byte:			; bank byte of each block 00 means "not inserted"
		incbin "/tmp/__banks.bin"	; 16KB -- can be made into a incsrc for manual use
	.end
endif
...
block_pointers_1:				; two byte pointer per block -- little endian as expected.
	incbin "/tmp/__pointers_1.bin"	; 32KB -- can also be made to be incsrced
.end

if !__insert_pages_40_plus == 1
	%bad_freedata_workaround("cleaned")
	block_pointers_2:
		incbin "/tmp/__pointers_2.bin"; 32KB -- can also be made to be incsrced
	.end
	%bad_freedata_workaround("cleaned")
	block_bank_byte:			; bank byte of each block 00 means "not inserted"
		incbin "/tmp/__banks.bin"	; 32KB -- can be made into a incsrc for manual use
	.end
endif
